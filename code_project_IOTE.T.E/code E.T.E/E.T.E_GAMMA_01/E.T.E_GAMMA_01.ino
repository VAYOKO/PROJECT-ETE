...
display.begin(SSD1306_SWITCHCAPVCC, 0x3C);  
    ...
    BLYNK_WRITE(V29)
  {
    int VAL = param.asInt();
    Serial.println(VAL);
    Blynk.virtualWrite(V30,"เชื่อมต่อเรียบร้อย E.T.E");
    Blynk.virtualWrite(V30,ssid);
     Blynk.virtualWrite(V30,pass);
  }
...
void gasDetect(void) {
  if (analogRead(Gas)>350){
    led4.on();
  delay(10);
Blynk.virtualWrite(V30,"!!ตรวจพบเเก๊ส!!");
    Serial.println("GAS, GAS, GAS");      
    Blynk.virtualWrite(V15,Gas);
  }
  else{
    led4.off();
    Blynk.virtualWrite(V15,Gas);
...
view rawldr_led.ino hosted with ❤ by GitHub
